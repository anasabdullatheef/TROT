...
Tests predictive power of on-chain data:
- Fear & Greed Index
- Exchange flow estimates
- Whale activity estimates
- Blockchain metrics (hashrate, tx count)
"""
...
from onchain_features import compute_onchain_features, get_onchain_feature_columns
from orderflow_features import compute_hmm_features, get_orderflow_feature_columns
from orderflow_features import calculate_funding_momentum
...
def get_combined_features() -> List[str]:
    """Combined on-chain + funding momentum features"""
    return [
        # Fear & Greed
        "fng_value",
        "fng_momentum",
        ...
        # Estimated on-chain flows
        "exchange_flow_est",
        "whale_activity_est",
        "miner_pressure_est",
        "exchange_reserve_change_est",

        # Funding momentum (from order flow)
        "funding_rate",
        "funding_8h_momentum",
...
class OnChainLSTM:
    """LSTM trained on on-chain features"""

    def __init__(self, regime_id: int, feature_columns: List[str] = None):
        self.regime_id = regime_id
        self.feature_columns = feature_columns or get_combined_features()
        self.model = None
    ...
    def train(self, df: pd.DataFrame, epochs: int = 50, batch_size: int = 32) -> Dict:
        """Train on on-chain features only"""
        ...
        self.trained = True
        return {"accuracy": self.accuracy, "auc": self.auc, "samples": len(X), "features": len(available)}
...
def train_onchain_models(train_df: pd.DataFrame) -> Tuple[RegimeDetector, Dict]:
    """Train HMM and on-chain LSTM models"""
    print("\n" + "=" * 70)
    print("TRAINING MODELS (ON-CHAIN FEATURES)")
    print("=" * 70)
    ...
    # LSTM per regime
    print("\n🔧 Training LSTM with on-chain features...")
    features = get_combined_features()
    ...
    for regime in range(config.HMM_N_STATES):
        model = OnChainLSTM(regime_id=regime, feature_columns=available)
...
def main():
    """Main execution"""
    print("🚀 ATLAS BOT - ON-CHAIN FEATURES BACKTEST")
    print("=" * 70)
    ...
    # On-chain features
    df = compute_onchain_features(ohlcv, include_api_data=True)

    # Add HMM features (for regime detection)
    ...
    print(f"✅ Total features: {len([c for c in df.columns if c not in ['timestamp', 'open', 'high', 'low', 'close', 'volume']])}")

    # Split
    train_df, test_df = split_train_test(df)

    # Train
    regime_detector, models, results = train_onchain_models(train_df)

    # AUC Report
    print("\n" + "=" * 70)
    print("📊 ON-CHAIN FEATURES AUC RESULTS")
    print("=" * 70)
    ...
    for regime, metrics in sorted(results.items()):
        ...
        if auc < 0.60:
            all_good = False
        print(f"Regime {regime}: AUC={auc:.3f} Acc={metrics['accuracy']:.2%} {status}")

    print("\n" + "-" * 50)
    if all_good:
        print("✅ All regimes AUC >= 0.60 - Strategy viable")
    else:
        print("⚠️ Some regimes AUC < 0.60 - Needs more work")
    ...
    plt.title(f"On-Chain Features Strategy - Return: {stats['return_pct']:.2f}%")
    plt.ylabel("Capital")
    plt.grid(True, alpha=0.3)
    plt.savefig("data/onchain_backtest.png", dpi=150, bbox_inches="tight")
    plt.close()
    print(f"\n📊 Chart saved to data/onchain_backtest.png")
